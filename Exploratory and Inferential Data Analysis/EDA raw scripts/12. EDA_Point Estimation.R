...
#3. Apply the calculate_avg function to each dataset
...
#4. Calculate the number of countries for each IncomeGroup
income_group_count <- domestic_with_info %>%
  group_by(IncomeGroup) %>%
  summarise(Count = n_distinct(Country.Code))

#5. Calculate the overall average (weighted by the number of countries in each IncomeGroup)
...
#6. Merge the IncomeGroup-level and overall averages into a final table
...
#7. Print the result (Display up to 200 rows)
