...
# Load datasets
data_list <- list(
  agriculture = "agriculture.rds", domestic = "domestic.rds", gdppc = "gdppc.rds",
  gdppc_from1993 = "gdppc_from1993.rds", industry = "industry.rds", info = "info.rds",
  natural_disasters = "natural_disasters.rds", population = "population.rds",
  private_investment = "private_investment.rds", water_productivity = "water_productivity.rds",
  water_stress = "water_stress.rds", withdrawals = "withdrawals.rds",
  available_resources = "available_resources.rds", gdp_ppp = "gdp_ppp.rds", gdp_usd = "gdp_usd.rds"
)

for (name in names(data_list)) assign(name, readRDS(data_list[[name]]))

# Function to reshape and merge datasets
...
# Reshape and merge all datasets
